// $Id: runHgg.C,v 1.4 2011/02/01 16:51:37 bendavid Exp $
...
#include "MitHgg/Mods/interface/PhotonEfficiency.h"
...
//--------------------------------------------------------------------------------------------------
void runPhEff(const char *fileset    = "0000",
	    ...
	    int         nEvents    = 1000)
{
  ...
  muonId->SetIsoType   ("NoIso");
  muonId->SetApplyD0Cut(kTRUE);

  PhotonIDMod         *photId = new PhotonIDMod;
  photId->                SetIsoType("NoIso");
  photId->                SetApplySpikeRemoval(true);
  photId->                SetApplyPixelSeed(false);
  photId->                SetHadOverEmMax(100.);
  photId->                SetPtMin(5.);
  photId->                SetEtaWidthEB(0.0105);
  photId->                SetEtaWidthEE(0.03);
  photId->                SetAbsEtaMax(10.);
  photId->                SetApplyR9Min(false);

  PhotonEffMod *phEffMod = new PhotonEffMod;
  phEffMod->SetMuonName       (muonId->GetOutputName());
  phEffMod->SetMuonsFromBranch(kFALSE);
  phEffMod->SetPhotonName       (photId->GetOutputName());
  phEffMod->SetPhotonsFromBranch(kFALSE);
  phEffMod->SetOverlapCut(double(overlapCut));

  if (jsonFile.CompareTo("/home/fabstoec/cms/json/~") != 0)
    phEffMod->SetIsData(kTRUE);
  else
    phEffMod->SetIsData(kFALSE);
