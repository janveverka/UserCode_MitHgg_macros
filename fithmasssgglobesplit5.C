...
void fithmasssgglobesplit5(int filterproc = 3) {//filterproc corresponds to a particular production mechanism
  //---set plot style---
  ...
  procnames.push_back("all");
  TString procname = procnames.at(filterproc);
  //---change directory---
  //gSystem->cd(TString::Format("./bambuOutputMar19smmet/%s",procnames.at(filterproc).Data())); 
  gSystem->cd(TString::Format("/home/mingyang/cms/root/RootFiles/hggmva_2012_jan16/SignalModel/%s",procnames.at(filterproc).Data()));  
  gStyle->SetOptStat(1110);

   //---roofit messenger options---
  ...
  TFile *fdata = new TFile("/home/mingyang/cms/root/RootFiles/hggmva_2012_jan16/SignalModel/CMS-HGG-mclow-2012jan16.root","READ"); 
  ...
  TFile *fdata2 = new TFile("/home/mingyang/cms/root/RootFiles/hggmva_2012_jan16/SignalModel/CMS-HGG-mchigh-2012jan16.root","READ");
  ...
  //---number of mass points to fit---
  RooRealVar mnom("MH","m_{h}",110.0,100.0,200.0,"GeV");
  mnom.setConstant();
 
  //---define categories and choose which combination used to fit each category---
  ...
  catnames.push_back(catnamesbase.at(0)+TString("_")+procnames.at(filterproc));//e.g cat0_ggh
  ...
  complexset.insert(catnamesbase.at(1)+TString("_")+procnames.at(filterproc));//e.g. cat1_ggh
  ...
  std::set<TString> simpleset;//no longer used
  //simpleset.insert("cat6");
  //simpleset.insert("cat7");
 
  //---signal model and parameter to fit---
  //the choice of the pdf and parameters are emperical
  //pdf component from right vertex
  ...
  //---pdf component from wrong vertex---
  ...
  //signal pdf used for different categories
  ...
  //get input discrete hisrogram for branching ratio and cross section as a function of mass
  ...
  //get the interpolated continuous hisrogram for branching ratio and cross section as a function of mass
  RooDataHist *dsmbrs = new RooDataHist("dsmbrs","",RooArgList(mnom),hsmbrs);//mnom:?
  ...
  //---make the combined dataset of all the cateogry for each mass point and import back to RooWorkspace---
  ...
  /*for (UInt_t i=0; i<mhs.size(); ++i) {
    ...
    for (UInt_t icat=0; icat<catnames.size(); ++icat) {
      RooDataSet *mcsigdata = (RooDataSet*)win->data(TString::Format("sig_%s_mass_m",procname.Data())+numstring+TString("_rv_")+catnamesbase.at(icat));//e.g.sig_ggh_mass_m110_rv_cat0
  ...
  TH2F *histfitstatus = new TH2F("histfitstatus","histfitstatus",(massmax-110.0)/10.0,105,massmax-5.0,catnames.size(),-0.5,catnames.size()-0.5);//0 fit; 1 not fit
  ...
  //---add canvas for plot---
  TCanvas* rightCan = new TCanvas("rightCan","right vertex",1250,1250); 
  rightCan->Divide(5,5);
  TCanvas* wrongCan = new TCanvas("wrongCan","wrong vertex",1250,1250); 
  wrongCan->Divide(5,5);
  TCanvas* allCan = new TCanvas("allCan","combined",1250,1250); 
  allCan->Divide(5,5);
  ...
  for (UInt_t icat=0; icat<catnames.size(); ++icat) {
     for (UInt_t i=0; i<mhs.size(); ++i) {
      ...
      if(filterproc!=4){
	dataRightName=TString::Format("sig_%s_mass_m",procname.Data())+numstring+TString("_rv_")+catnamesbase.at(icat);
        dataWrongName=TString::Format("sig_%s_mass_m",procname.Data())+numstring+TString("_wv_")+catnamesbase.at(icat);
	...
	//continue;
      ...
      //select the pdf and set the parameters
	...
	fitreswrong = wrongpdf->fitTo(*mcsigwrongwdata,Strategy(0),Minimizer("Minuit2",""),Minos(kFALSE),SumW2Error(kFALSE), Save(kTRUE),NumCPU(8));    
        ...
        //if (!fitres->status()) fitres = rightpdf->fitTo(*mcsigallwdata,Strategy(1),Minimizer("Minuit2",""),Minos(kFALSE),SumW2Error(kFALSE), Save(kTRUE),NumCPU(8));//what does this mean? 
        ...
        //fitreswrong = fitres;
        fitres = rightpdf->fitTo(*mcsigallwdata,Strategy(2),Minimizer("Minuit2",""),Minos(kFALSE),SumW2Error(kFALSE), Save(kTRUE),NumCPU(8)); 
        fitreswrong = fitres;
      }
      ...
      //rearrange the order of the paprameter so that the gaussian width increases in the order g1, g2, g3
      ...
      //fill histograms with fitted parameters and fit status in 10 GeV steps
      if ( (mhs.at(i)%10) == 0) {
        for (UInt_t iparm=0; iparm<fitparms.size(); ++iparm) {
	...
	histfitstatus->Fill(mhs.at(i),icat,fitres->status());
      ...
      if ( (mhs.at(i)%10) == 0) {
	if((mhs.at(i)<115)){
	  rightCan->cd(icat*5+i+1);
	  hplot->Draw();  
	  wrongCan->cd(icat*5+i+1);
	  hplotwrong->Draw(); 
	  allCan->cd(icat*5+i+1);
	  hplotall->Draw(); 
	}
        if((mhs.at(i)>115)){
	  rightCan->cd(icat*5+i);
	  hplot->Draw();  
	  wrongCan->cd(icat*5+i);
	  hplotwrong->Draw(); 
	  allCan->cd(icat*5+i);
	  hplotall->Draw(); 
	}
  ...
  //---construct interpolation function for each fit parameter in each category---
  ...
  for (UInt_t icat=0; icat<catnames.size(); ++icat) {
    for (UInt_t iparm=0; iparm<fitparms.size(); ++iparm) {
      ...
      fitparmfuncs[icat*nparms + iparm] = new RooHistFunc(funcname,funcname,RooArgList(mnom),*fitparmdatas[icat*nparms + iparm],1);//a function of mnom      
    }
  }
  ...
  for (UInt_t icat=0; icat<catnames.size(); ++icat) {
    for (UInt_t iparm=0; iparm<fitparms.size(); ++iparm) {
      TString plotname = TString("func") + TString(fitparms.at(iparm)->GetName()) + catnames.at(icat) + TString(".eps");//e.g funcsigma1cat0_ggh.eps
...
//   nuissancedeltamcors[7] = deltammixedmixedr9;
  ...
  for (UInt_t icat=0; icat<catnames.size(); ++icat) {
    ...
    sigma1slides[icat] = new RooFormulaVar(TString("sigma1slide")+catnames.at(icat),"","TMath::Max(0.01,sqrt(@0*@0-@3*@3*@2*@2 +@1*@1))",RooArgList(*fitparmfuncs[icat*nparms+3],*smearmods[icat],*smears[icat],mnom));//why set min 0.01?
